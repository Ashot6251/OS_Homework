...
0000000000000000 <main>:
   ...
   c:   c7 45 f8 0a 00 00 00    movl   $0xa,-0x8(%rbp)
...
Based on the outputs from the nm command, the main.o object file defines the main symbol marked as T located in the text section but lists printf and square as undefined (U) because their actual implementations exist outside of this file. The math_utils.o file successfully defines the square symbol. Once linked into the final square_prog executable, both main and square are resolved and assigned concrete memory addresses within the executable's text section.

The primary role of the linking process is to resolve independent references and combine the separated code into a single runnable program. In the objdump assembly for main.o, the compiler uses placeholder offset addresses for the square and printf function calls because it does not yet know where those functions will reside in memory. During linking, the object files are combined, and the linker replaces these placeholder offsets with calculated memory addresses.

The readelf headers reveal that the object files are relocatable (REL) files. This means they contain compiled machine code and data, but they are not yet fully assembled into a runnable state. Their memory addresses are not fixed, and their entry point addresses are simply set to 0x0. Using the -S option shows that these object files consist of basic sections like .text for code and .data/.bss for variables. The sections inside these relocatable files act as separate blocks. When the linker combines them into the final square_prog executable, the file type changes to a position-independent executable (DYN). The final executable merges these sections together and assigns an actual entry point address (0x1060) to prepare the program to run.
